...
#' 
#' safetyGraphics:::getSettingsMetadata(text_keys=c("id_col"),dmeta=labs_meta)
#' # returns a dataframe with a single row with metadata for the id_col setting
#'
#' safetyGraphics:::getSettingsMetadata(text_keys=c("id_col"), cols=c("label"), dmeta=labs_meta)
...
getSettingsMetadata<-function(charts=NULL, text_keys=NULL, cols=NULL, filter_expr=NULL, add_standards=TRUE, dmeta=NULL){
  md <- dmeta$settings %>% mutate(text_key=as.character(.data$text_key))

  if(add_standards){
    ms<-dmeta$standards %>% mutate(text_key=as.character(.data$text_key))
