...
#' @param domain_metadata domain level data, like that generated by \code(generateMetadata(domain="labs"))
#' 
#' @examples
#' \dontrun{
#'
...
#'       domain_metadata = getMetadata(domain="labs"))
#'
#' }
#'
#' @import htmlwidgets
#'
#' @export
chartRenderer <- function(data, debug_js = FALSE, settings = NULL, chart=NULL, domain_metadata=NULL) {
  ...
  #Set Chart Width
  chartMaxWidth<-  domain_metadata$charts %>% filter(.data$chart==!!chart) %>% pull(.data$maxWidth)
  settings$max_width <- chartMaxWidth

  #Renderer
  chartFunction<- domain_metadata$charts %>% filter(.data$chart==!!chart) %>% pull(.data$main)
  subFunction<-  domain_metadata$charts %>% filter(.data$chart==!!chart) %>% pull(.data$sub)
  chartType <-  domain_metadata$charts %>% filter(.data$chart==!!chart) %>% pull(.data$type)
