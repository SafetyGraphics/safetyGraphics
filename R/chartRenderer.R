...
#' @param dmeta domain level data, like that generated by \code{generateMetadata(domain="labs")}
#' 
#' @examples
#' \dontrun{
#'
...
#'
#' @export
chartRenderer <- function(data, debug_js = FALSE, settings = NULL, chart=NULL, dmeta=NULL) {
  ...
  #Set Chart Width
  chartMaxWidth<-  dmeta$charts %>% filter(.data$chart==!!chart) %>% pull(.data$maxWidth)
  settings$max_width <- chartMaxWidth

  #Renderer
  chartFunction<- dmeta$charts %>% filter(.data$chart==!!chart) %>% pull(.data$main)
  subFunction<-  dmeta$charts %>% filter(.data$chart==!!chart) %>% pull(.data$sub)
  chartType <-  dmeta$charts %>% filter(.data$chart==!!chart) %>% pull(.data$type)
