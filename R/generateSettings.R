...
#'
#' This function returns a settings object for the requested chart(s) based on the specified data standard.
#'
#' The function is designed to work with the SDTM and ADaM CDISC(<https://www.cdisc.org/>) standards for clinical trial data.
#'
...
#' @param useDefaults Specifies whether default values from dmeta$settings should be included in the settings object. Default: \code{TRUE}.
#' @param partial Boolean for whether or not the standard is a partial standard. Default: \code{FALSE}.
#' @param partial_keys Optional character vector of the matched settings if partial is TRUE. Settings should be identified using the text_key format described in \code{?metadata}. This setting is ignored when partial is FALSE. Default: \code{NULL}.
...
#' generateSettings(standard="SDTM", dmeta=labs_meta)
#' generateSettings(standard="SdTm", dmeta=labs_meta) #also ok
#' generateSettings(standard="ADaM", dmeta=labs_meta)
#' pkeys<- c("id_col","measure_col","value_col")
#' generateSettings(standard="adam", partial=TRUE, partial_keys=pkeys, dmeta=labs_meta)
#'
#' generateSettings(standard="a different standard", dmeta=labs_meta)
...
generateSettings <- function(standard="None", charts=NULL, useDefaults=TRUE, partial=FALSE, partial_keys=NULL, custom_settings=NULL, dmeta=NULL){
  ...
  standardList<-dmeta$standards%>%select(-.data$text_key)%>%names

  if(standard %in% standardList){
    dataDefaults <- getSettingsMetadata(
      charts = charts,
  ...
  #############################################################################
  if(useDefaults){
    otherDefaults <- getSettingsMetadata(charts = charts, dmeta=dmeta) %>% 
  ...
  #############################################################################
  shell<-generateShell(charts=charts, dmeta=dmeta)
  ...
  if (nrow(key_values)>0){
    for(row in 1:nrow(key_values)){
      ...
      type <- getSettingsMetadata(text_keys=text_key,cols="setting_type",dmeta=dmeta)
