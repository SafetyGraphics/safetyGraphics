...
#' safetyGraphics:::evaluateStandard(data=labs, standard="adam", dmeta=labs_meta) # Match is TRUE
#' safetyGraphics:::evaluateStandard(data=labs, standard="sdtm", dmeta=labs_meta) # Match is FALSE
#'
#' @import dplyr
...
evaluateStandard <- function(data, standard, includeFields=TRUE, dmeta=NULL){
  ...
  col_list<-c("text_key", "column_mapping", "field_mapping", "field_column_key", "setting_required","standard_val",standard)
  
  standardChecks <- getSettingsMetadata(cols=col_list, dmeta=dmeta) %>%
  ...
  rowwise %>%
  mutate(field_column_name = ifelse(.data$field_mapping, getSettingsMetadata(cols=standard, text_keys=.data$field_column_key, dmeta=dmeta),"")) %>%
  mutate(
