...
#' v1<-validateSettings(data=labs, settings=testSettings,dmeta=labs_meta)
#' # v1$valid is TRUE
#' 
#' testSettings$id_col <- "NotAColumn"
#' validateSettings(data=labs, settingsMetadata=settingsMeta)
#' # .$valid is now FALSE
#'
#' @export
#' @import dplyr
...
validateSettings <- function(data, settings, charts=NULL, dmeta){
  #initialize shell settings
  settingStatus<-list()

  # if no charts are specified, use all available
  if (is.null(charts)){
    charts <- dmeta$charts$chart
  }

  # Check that all required parameters are not null
  requiredChecks <- getRequiredSettings(charts = charts, dmeta=dmeta) %>% purrr::map(checkRequired, settings = settings)
  ...
  if (!is.null(allKeys)){
    ...
    fieldChecks <- fieldKeys %>% purrr::map(checkField, settings=settings, data=data, dmeta=dmeta )
  }

  #Check that settings for mapping numeric data are associated with numeric columns
  numericChecks <- NULL
  numericKeys <- getSettingsMetadata(charts=charts, filter_expr=.data$column_type=="numeric", cols="text_key", dmeta=dmeta)
  ...
  for(chart in charts){
    if(settingStatus$valid){
      ...
      chart_keys <- getSettingsMetadata(charts=chart, cols ="text_key", dmeta=dmeta)
