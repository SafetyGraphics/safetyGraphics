(function (global, factory) {
        ...
        ['raw', 'custom'].forEach(function (property) {
            ...
            switch (_this.config.x.bin_algorithm) {
                case 'Square-root choice':
                    ...
                    obj.stats.nBins = obj.stats.SquareRootBins < obj.stats.nUnique ? obj.stats.SquareRootBins : obj.stats.nUnique;
                    break;
                case "Sturges' formula":
                    calculateSturgesBinWidth.call(_this, obj);
                    obj.stats.nBins = obj.stats.SturgesBins < obj.stats.nUnique ? obj.stats.SturgesBins : obj.stats.nUnique;
                    break;
                case 'Rice Rule':
                ...
                case "Scott's normal reference rule":
                    ...
                    obj.stats.nBins = obj.stats.ScottBins < obj.stats.nUnique ? obj.stats.ScottBins : obj.stats.nUnique;
                    break;
    ...
    function updateXAxisLimits() {
        ...
        .style('box-shadow', 'none').property('value', this.config.x.d3format1(this.config.x.domain[0]));

        this.controls.wrap.selectAll('#upper input').attr('step', this.measure.step) // set in ./calculateXPrecision
        .style('box-shadow', 'none').property('value', this.config.x.d3format1(this.config.x.domain[1]));
    }

    function updateBinAlogrithm() {
        this.controls.Algorithm.selectAll('.algorithm-explanation').style('display', this.config.x.bin_algorithm !== 'Custom' ? null : 'none').attr('title', this.config.x.bin_algorithm !== 'Custom' ? 'View information on ' + this.config.x.bin_algorithm : null);
    }

    function updateBinWidth() {
        this.controls.Width.selectAll('input').property('value', this.config.x.d3format1(this.config.x.bin_width));
    }
    ...
    function drawZeroRangeBar() {
        ...
        if (this.current_data.length === 1 && this.measure.filtered.domain[0] === this.measure.filtered.domain[1]) {
    ...
    function addHoverBars() {
        ...
        var bins = this.svg.selectAll('.bar-group').each(function (d) {
            ...
            d.footnote = '<span style = \'font-weight: bold\'>' + d.values.raw.length + ' records</span> with ' + (context.measure.current + ' values &ge;<span style = \'font-weight: bold\'>') + (context.config.x.d3format1(d.rangeLow) + '</span> and ' + (d.rangeHigh < context.config.x.domain[1] ? '&lt;' : '&le;') + '<span style = \'font-weight: bold\'>' + context.config.x.d3format1(d.rangeHigh) + '</span>');
        });
    }

    function mouseout(element, d) {
        //Update footnote.
        this.footnotes.barDetails.html(this.highlightedBin ? 'Table displays ' + this.highlighteD.footnote + '.' : '');
    ...
    function annotateBinBoundaries() {
        ...
        if (this.config.annotate_bin_boundaries) {
            ...
            texts.each(function (d) {
                ...
                })) text.remove();else textDimensions.push({
                    x: bbox.x,
                    width: bbox.width,
                    y: bbox.y,
                    height: bbox.height
