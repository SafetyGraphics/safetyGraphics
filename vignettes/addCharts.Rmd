...
1. Create customSettings.R script
2. Make static code using sample data
3. Covert static chart code to Function

### Step 1.1 - Create customSettings.R Script

First, Create a file called customSettings.R and save it in your working directory. All of the code in the following steps goes in this file. Note that when you run the app, custom medatadata files will be saved in your working directory, so you might want to call setwd() and put this somewhere out of the way. 

### Step 1.2 - Make Static Code Using Sample Data

Next, write some sample code for your chart using a sample data set (the `adlbc` data set included with safetyGraphics is a good option). Note that all common charting packages are supported including `ggplot2`, `lattice` and base R graphics.  Our example makes a histogram showing the distribution of lab values: 
...
After the chart is working on the sample data, you need to update it to a function that will work with any data set that the user loads in the shiny app.  Replace hard coded parameters with references to the settings defined in the safetyGraphics metadata object as shown below. 
...
- Finally, note that the process for defining custom "htmlwidget" and "shiny module" charts is slightly different than the sample code above; examples 3 provides basic example for a shiny module, and htmlwidgets are discussed in Appendix 2. 
...
First, we need to determine which settings are already defined. As noted above, our chart uses 3 settings: `value_col`, `measure_col` and `box_color`. In most cases you can just examine the  `settingsMetadata` data frame. Either view it with view(settingsMetadata) or use code like this:
...
In some complex cases, it might be easier to examine the metadata object directly. To do this, you can create a shell settings object and then check to see if the settings exist:
